"""Batch builder for the automotive_sanfrancisco_v2 classify stage.

Builds review_index.json (id -> {place_id, rating, reviewer, snippet}) and
batch_NNN.json files (each a list of {id, snippet, rating}) for the
pain-classifier subagent. Selection source = master independents (not
is_chain_or_dso); reviews source = raw NDJSON. Follows the /outreach classify
runbook (merge user_reviews + extended, dedupe, rating<=3 filter, FULL review
text, batches of BATCH_SIZE). Mirrors automotive_sanfrancisco/_classify_work.

Campaign-relative and date-agnostic: resolves its own campaign root and the
latest outputs/<date>/master.json, so it needs no editing across deliveries.
"""
from __future__ import annotations
import json, math
from pathlib import Path

CAMP = Path(__file__).resolve().parents[2]   # campaigns/automotive_sanfrancisco_v2
WORK = Path(__file__).resolve().parent
BATCH_SIZE = 40


def latest_master(camp: Path) -> Path:
    dated = sorted(
        (p for p in (camp / 'outputs').iterdir() if (p / 'master.json').exists()),
        key=lambda p: p.name,
    )
    if not dated:
        raise FileNotFoundError(f'no outputs/<date>/master.json under {camp}')
    return dated[-1] / 'master.json'


def only_raw(camp: Path) -> Path:
    files = sorted((camp / 'raw').glob('*.json'))
    if not files:
        raise FileNotFoundError(f'no raw/*.json under {camp}')
    return files[0]


RAW = only_raw(CAMP)
MASTER = latest_master(CAMP)
...
for lead in independents:
    ...
    for rev in merged:
        ...
        rows.append({'id': rid, 'snippet': text, 'rating': rating})
        kept += 1
    if kept:
...
print(f'batches      -> {batch_dir}/batch_000.json .. batch_{max(n_batches-1,0):03d}.json')
