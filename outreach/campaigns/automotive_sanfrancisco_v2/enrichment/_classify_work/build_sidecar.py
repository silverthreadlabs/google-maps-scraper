...
Mirrors automotive_sanfrancisco/_classify_work/build_sidecar.py.

Campaign-relative and date-agnostic: derives the sidecar date from the latest
outputs/<date>/ folder so it matches the master being enriched.
"""
...
CAMP = WORK.parents[1]   # campaigns/automotive_sanfrancisco_v2
review_index = json.load(open(WORK / 'review_index.json'))
batch_dir = WORK / 'batches'
out_dir = WORK / 'batches_out'


def latest_date(camp: Path) -> str:
    dated = sorted(
        (p.name for p in (camp / 'outputs').iterdir() if (p / 'master.json').exists())
    )
    if not dated:
...
# write sidecar
date = latest_date(CAMP)
sidecar_path = CAMP / 'enrichment' / 'pain_classifications' / f'{date}.json'
