"""Batch builder for the automotive_sanfrancisco classify stage.
...
subagent. Selection source = master independents (not is_chain_or_dso);
reviews source = raw NDJSON. Follows the /outreach classify runbook (merge
user_reviews + extended, dedupe, rating<=3 filter, full review text,
batches of BATCH_SIZE). Mirrors automotive_dallas/_classify_work/build_batches.py.
"""
from __future__ import annotations
import json, math
from pathlib import Path

CAMP = Path(__file__).resolve().parents[2]   # campaigns/automotive_sanfrancisco
RAW = CAMP / 'raw' / 'automotive_sf_bay.json'
MASTER = CAMP / 'outputs' / '2026-06-15' / 'master.json'
